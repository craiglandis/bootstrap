...
md c:\my
curl https://raw.githubusercontent.com/craiglandis/bootstrap/main/Set-Wallpaper.ps1 c:\my
(New-Object Net.Webclient).DownloadFile('https://raw.githubusercontent.com/craiglandis/ps/master/Get-FirewallRulesFromRegistry.ps1', "$env:SystemDrive\Get-FirewallRulesFromRegistry.ps1")
#>
...
$hasNvidiaGPU = [bool]($gpus | where AdapterCompatibility -eq 'NVIDIA')
if ($hasNvidiaGPU)
{
    ...
    if ($hasNvidiaSmi)
    {
        ...
        $gpus | where AdapterCompatibility -eq 'NVIDIA' | foreach-object {
...
foreach ($displayClassSubkeyName in $displayClassSubkeyNames) {
    ...
    if ($matchingDeviceId)
    {
        ...
        $gpus | where {$_.PNPDeviceID.Contains($matchingDeviceId)} | ForEach-Object {
...
else
{
    ...
    if (($gpu | measure-object | select -ExpandProperty Count) -gt 1)
    {
...
if ((Test-Path -Path $myFolderPath -PathType Container) -eq $false)
{
    New-Item -Path $myFolderPath -ItemType Directory -Force | Out-Null
}
...
if ($scriptFullName.StartsWith('\\tsclient'))
{
    ...
    $localScriptPath = "$myFolderPath\$scriptName"
...
$setWallpaperVbsPath = $scriptFullName.Replace('.ps1','.vbs').Replace('\\tsclient\c','c:')
...
Set-Alias w Set-Wallpaper2.ps1
if (Test-path -path $profile -PathType Leaf)
{
	$usingProfileInOnedrive = get-content $profile | Select-String -SimpleMatch 'C:\OneDrive\my\Profile.ps1' -Quiet
	if ($usingProfileInOnedrive -eq $false)
	{
		$profileAlreadyUpdated = get-content $profile | Select-String -SimpleMatch 'Set-Alias w Set-Wallpaper.ps1' -Quiet
		if ($profileAlreadyUpdated -eq $false)
		{
			Add-Content -Value 'Set-Alias w Set-Wallpaper.ps1' -Path $profile -Force
		}
	}
}
